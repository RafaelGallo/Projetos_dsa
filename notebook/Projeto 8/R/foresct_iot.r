...
serie_temporal <- ts(data$Appliances, frequency = 24) # Supondo que a frequência dos dados seja diária (24 observações por dia)

# Divida a série temporal em conjunto de treinamento e teste
tamanho_treinamento <- floor(0.5 * length(serie_temporal))
...
# Modelo ARIMA
library(forecast)
modelo_arima <- auto.arima(tamanho_treinamento)
modelo_arima
